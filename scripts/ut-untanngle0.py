...
def text_region_handler(node, begin_index, end_index, annotations):
    ...
    if 'iiif_url' in node['metadata']:
        scan_annot_info = {'resource_id': resource_id, 'type': AnnTypes.SCAN.value,
                           'iiif_url': node['metadata']['iiif_url'], 'begin_anchor': begin_index,
                           'end_anchor': end_index, 'id': node['metadata']['scan_id']}
        annotations.append(scan_annot_info)

        page_annot_info = {'resource_id': resource_id, 'type': AnnTypes.PAGE.value, 'begin_anchor': begin_index,
                           'end_anchor': end_index, 'id': node['metadata']['page_id'],
                           'metadata': {'page_id': node['metadata']['page_id'], 'scan_id': node['metadata']['scan_id']},
                           'coords': node['coords']}
    ...
    for p in paras:
        ...
        for lr in p['line_ranges']:
            ...
            if line_begin <= att_end < line_end:
                ...
                result = {'begin_anchor': begin_anchor, 'end_anchor': end_anchor,
